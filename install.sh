...
#--------------------------
# Installing OpenShift Serverless operator (optional)
#--------------------------
...
#--------------------------
# Installing namespaced operators: Camel-K, Datagrid, AMQ Stream, AMQ Broker is optional in case of Drogue deployment
#--------------------------
if [[ "$INSTALL_OPERATORS" == true ]]; then
  ...
  #--------------------------
  # Waiting until all operator deployments are available
  #--------------------------
  if [[ "$DROGUE_IOT" != true ]]; then
...
#--------------------------
# Installing the Infra Helm chart, Drogue Cloud is optional 
#--------------------------
...
#--------------------------
# Waiting until infra components are running 
#--------------------------
if [[ "$DROGUE_IOT" != true ]]; then
...
#--------------------------
# Installing the Applications Helm Chart
#--------------------------
...
#--------------------------
# Waiting until some core app components are running 
#--------------------------
...
#--------------------------
# Installing the Serverless Helm Chart
#--------------------------
...
--set-string namespace="$NAMESPACE" \
--set drogueIoT="$DROGUE_IOT" \
--set-string otaServer.url="http://ota-server-$NAMESPACE.$APP_DOMAIN"

#--------------------------
# Waiting until Camel-K integrations are build and running (takes around 5-10 minutes) 
#--------------------------
